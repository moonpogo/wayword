...
| First-session entry run 1 post-submit | Early first-session submits use the Entry prompt layer and low-stakes post-run behavior. Below-editor Mirror remains statement-only and non-diagnostic. | `script.js`, `src/app/prompt-runtime.js`, `src/ui/render-post-run.js`, `src/features/writing/completion-aftermath-helper.js` |
| First-session entry threshold handoff | When the saved run crosses `FIRST_SESSION_ENTRY_THRESHOLD` and handoff is not acknowledged, the first-session handoff section is shown. Normal completed restart shortcuts are blocked while handoff is visible. | `script.js`, `src/features/writing/successful-submit-coordinator.js`, `src/features/writing/completed-ui-restart-interactions.js` |
| Handoff Continue | Acknowledges `wayword-firstSessionEntry-handoff-ack`, hides handoff, and starts a fresh run with caret at start. | `script.js` |
...
| Normal post-first-session Mirror submit | Later submits render statement-only Mirror output when available, optional recent-trends block, and next-pass nudge. No visible evidence toggles are part of V1. | `mirror-engine.iife.js`, `mirror-dom.js`, `src/ui/render-post-run.js`, `script.js` |
...
| Clear saved runs | Patterns footer opens confirmation modal. Confirm resets first-session entry/saved-run state, clears canonical run docs when repo exists, persists empty legacy history/run ids, and returns app to fresh first-session posture. | `script.js`, `src/data/storage.js`, `src/data/runs/runDocumentRepository.js` |

## Needs Browser Verification

- Exact DOM/class snapshots for first-session entry post-submit, threshold handoff, and low-signal Mirror.
