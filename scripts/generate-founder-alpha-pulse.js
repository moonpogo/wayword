...
async function loadUsersAndRuns(supabase) {
  const [usersResult, runsResult, observatoryResult] = await Promise.all([
    supabase.from("users").select("id,created_at", { count: "exact" }),
    supabase.from("runs").select("user_id,local_created_at,created_at,updated_at", { count: "exact" }),
    supabase.from("observatory_summaries").select("user_id,generated_at"),
  ]);

  if (usersResult.error) throw new Error(`users query failed: ${usersResult.error.message}`);
  if (runsResult.error) throw new Error(`runs query failed: ${runsResult.error.message}`);
  if (observatoryResult.error) throw new Error(`observatory_summaries query failed: ${observatoryResult.error.message}`);

  return {
    users: Array.isArray(usersResult.data) ? usersResult.data : [],
    runs: Array.isArray(runsResult.data) ? runsResult.data : [],
    observatory: Array.isArray(observatoryResult.data) ? observatoryResult.data : [],
    totalAccounts: Number(usersResult.count || 0),
    totalRuns: Number(runsResult.count || 0),
  };
}
...
async function loadTelemetryEvents(supabase) {
  for (const table of TELEMETRY_TABLE_CANDIDATES) {
    ...
    if (!result.error) {
      ...
      return { available: true, table, rows: filtered };
    }
  }

  return { available: false, table: "", rows: [] };
}
...
function buildPulse(input) {
  ...
  const runCountYesterdayByUser = new Map();
  ...
  for (const row of runs) {
    ...
    if (isWithinWindow(stamp, startMs, endMs)) {
      incrementMapCounter(runCountYesterdayByUser, userId);
  ...
  const accountIds = new Set(users.map((u) => String(u.id || "")).filter(Boolean));
  const userIdsWithRuns = new Set(Array.from(runCountByUser.keys()));
  ...
  const usersWith3Runs = Array.from(runCountByUser.values()).filter((n) => n >= 3).length;
  const usersWith5Runs = Array.from(runCountByUser.values()).filter((n) => n >= 5).length;
...
function buildMarkdown(dateLabel, windowStartIso, windowEndIso, pulse) {
  ...
  if (!pulse.telemetryAvailable) {
    warnings.push("telemetry source unavailable");
  } else {
  ...
  if (pulse.telemetryAvailable) {
    ...
    lines.push("- Telemetry source unavailable.");
  }
