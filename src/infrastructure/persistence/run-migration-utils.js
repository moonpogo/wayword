(function () {
  ...
  function normalizeRunFingerprintInput(run) {
    var timestamp = Number(run && (run.savedAt || run.timestamp || run.createdAt || 0));
    var wordCount = Number(run && (run.wordCount || 0));
    var text = safeString(run && (run.originalText || run.text || ""));
    return {
  ...
  function classifyOverlap(localRun, serverRun) {
    ...
    var serverFingerprint = safeString(serverRun && serverRun.migration_fingerprint) || buildRunFingerprint(serverRun);
